...
- **고유명사 한글 표기는 통일합니다.** 같은 대상은 글 전체와 모든 포스트에서 한 가지 표기만 씁니다. 표준 표기는 아래와 같습니다.

| 대상 | 표준 표기 | 금지 표기 |
|------|-----------|-----------|
| Anthropic | 앤트로픽 | 앤쓰로픽, 앤스로픽, 엔쓰로픽 |
...
**고유명사 오표기 검출 패턴** (자가 검수용, 발견 시 표준 표기로 교체):
...
**참고자료**: APA 스타일, `<ul class="references-list">` 목록. 형식: `저자/매체. (연도, 월). <em>제목</em> [Article]. 플랫폼. (원문 보기 ↗)`. **날짜는 영어 표기**입니다(`(2026, June)` / `(2026, June 18)`, 한국어 월 금지). 노출 URL이나 도메인을 링크 텍스트로 쓰지 않고, 인용문 끝에 괄호로 `(<a href="{URL}" target="_blank" rel="noopener">원문 보기 ↗</a>)`를 붙입니다(화살표는 우상향 `↗`). 영상·팟캐스트·X 포스트도 `원문 보기 ↗`로 통일합니다. 소스가 여러 개이면 각각 `<li>`로 표기하고 항목마다 링크를 붙입니다. 참고자료 아래에 별도 `원문 보기 →` CTA 버튼을 두지 않습니다.
