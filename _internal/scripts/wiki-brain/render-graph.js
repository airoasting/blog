...
function renderGraph(rootDir, outPath) {
  ...
  // 출력 파일은 insights/wiki/graph.html(깊이 2), 원본은 insights/graph.html(깊이 1).
  // D3 스크립트 마커 앞 부분(head + body)만 가져오므로 그 안의 루트 상대경로를 한 단계 더 올린다.
  const scriptStart = baseHtml.indexOf('<!-- D3.js v7 -->');
  if (scriptStart === -1) throw new Error('Could not find D3.js script marker in insights/graph.html');
  let html = baseHtml.slice(0, scriptStart);
