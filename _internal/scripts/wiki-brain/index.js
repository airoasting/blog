...
const ALIASES_PATH = path.join(ROOT, '_internal/config/concept-aliases.json');
const DATA_PATH = path.join(ROOT, 'insights/wiki/concepts-data.json');
const REPORT_PATH = path.join(ROOT, 'insights/wiki/build-report.json');
...
async function main() {
  ...
  writeConceptPages(concepts, { postsBySlug, relationships }, path.join(ROOT, 'insights/wiki/concepts'));
  console.log(`✓ rendered ${concepts.length} concept pages`);

  renderIndex(concepts, path.join(ROOT, 'insights/wiki/index.html'), data.stats);
  renderGraph(ROOT, path.join(ROOT, 'insights/wiki/graph.html'));
  console.log('✓ rendered insights/wiki/index.html and insights/wiki/graph.html');
