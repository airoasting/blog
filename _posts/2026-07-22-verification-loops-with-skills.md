...
**첫째, 검증 루프의 정의가 분명합니다.** 원문은 이를 에이전트가 자기 작업을 스스로 점검하는 반복 주기로 규정합니다. 테스트를 돌리거나 검사 도구를 실행하거나 별도 점검을 수행하고, 실패한 것을 고친 뒤 다음으로 넘어갑니다. 사람이 끼어드는 지점이 사라지는 구조입니다.
