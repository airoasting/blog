...
```
소스 URL → /create-post → (자동) /edit-post → (자동) /publish-post
썸네일 재생성 → /generate-thumbnail (단독 실행)
```
...
- 개별 실행도 가능: `/edit-post <파일>`, `/publish-post <파일>`
...
| `/generate-thumbnail <slug>` | 로컬 스크립트로 썸네일 생성 또는 재생성 |
