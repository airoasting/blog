...
- **제목은 명사구 헤드라인**: 본문과 달리 제목은 `~습니다`/`~입니다`로 끝내지 않는다. 명사구나 압축 서술로 끝내고 40자 이내(20~30자 목표)로 쓴다. 대구 반복·반전 공식·2인칭 호출을 쓰지 않는다
...
| 칼럼 | 매체 기고문 원문 재수록 | `#4A2A6A` | `column/` |

> **칼럼은 문체 규칙의 예외입니다.** 전자신문 등 지면에 이미 실린 기고문을 원문 그대로 재수록하는 아카이브입니다. 종결어미(~입니다), 문장 길이 같은 블로그 규칙을 적용하지 않고 지면 문장을 손대지 않습니다. 본문 섹션은 `칼럼 전문`과 `기고문` 둘뿐입니다. 출처는 히어로의 소스 줄(매체명·기고일·원문 링크)과 하단 `기고문` 섹션의 인용으로 밝히고, 편집자 주석 섹션은 따로 두지 않습니다. 매체가 붙인 바이라인(소속·이메일)도 옮기지 않습니다. 3줄 요약·Roasting·페르소나 댓글은 붙이지 않습니다.
