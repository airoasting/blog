...
> **칼럼은 문체 규칙의 예외입니다.** 전자신문 등 지면에 이미 실린 기고문을 원문 그대로 재수록하는 아카이브입니다. 종결어미(~입니다), 문장 길이 같은 블로그 규칙을 적용하지 않고 지면 문장을 손대지 않습니다. 대신 본문 위에 `지면 정보` 섹션을 두어 매체명, 발행일, 지면 면수, 원문 링크를 밝힙니다. 편집자 주석(지면 정보 섹션)은 블로그 문체를 씁니다. 3줄 요약·Roasting·페르소나 댓글은 붙이지 않습니다.
